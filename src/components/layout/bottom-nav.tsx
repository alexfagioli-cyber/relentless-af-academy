...
const navItems = [
  {
    ...
    href: '/',
    icon: (active: boolean) => (
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill={active ? '#F59E0B' : '#8BA3C4'} className="w-5 h-5">
...
export function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 pb-safe" style={{ backgroundColor: '#122240', borderTop: '1px solid #1E3A5F' }}>
      <div className="flex items-center justify-around h-16 max-w-lg mx-auto">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              {item.icon(isActive)}
              <span className="text-xs" style={{ color: isActive ? '#F59E0B' : '#8BA3C4' }}>
                {item.label}
              </span>
            </Link>
          )
